...
バージョン 1.1.0 では `fetch-mode` が追加されました。

- `basic` : 検索結果一覧から取得できる項目のみ
- `detail` : 各ジャーナルのプロフィールページを取得して追加情報を補完
...
- `--fetch-mode basic|detail` を追加
- `basic` は 1.0.1 の主要項目を維持
- `detail` はプロフィール詳細ページで補完
...
- `basic` モードでは、検索結果一覧から安定して取得できる項目だけを出力します。
  - `journal_name`
  - `sinta_level`
  - `affiliation`
  - `journal_id`
  - `profile_url`
- `detail` モードでは、詳細ページを取得して追加項目も返します。
  - `p_issn`
  - `e_issn`
  - `subject_area`
  - `website_url`
  ...
  - `garuda_url`
  - `google_scholar_url`
- Google Scholar URL は、外部で補正せず **SINTA に登録されている値をそのまま返します**。
